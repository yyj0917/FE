...
import { FineDust, UvIndex } from '@/interfaces/weather.types';
 ...
 * @returns 러닝 지수 정보
 */
export function useRunningCondition(weatherData: WeatherData): RunningCondition {
  return useMemo(() => {
    const { temperature, windSpeed, fineDust, uvIndex } = weatherData;

    // 각 요소별 점수 계산
    const temperatureScore = calculateTemperatureScore(temperature);
    const windSpeedScore = calculateWindSpeedScore(windSpeed);
    const fineDustScore = calculateFineDustScore(fineDust);
...
// 온도 점수 계산
function calculateTemperatureScore(temperature: number): { score: number; status: string } {
  if (temperature >= 10 && temperature <= 20) {
    return { score: 100, status: '최적' };
  } else if (temperature >= 5 && temperature < 10) {
    return { score: 80, status: '쌀쌀' };
  } else if (temperature > 20 && temperature <= 25) {
    return { score: 75, status: '따뜻' };
...
// 풍속 점수 계산
function calculateWindSpeedScore(windSpeed: number): { score: number; status: string } {
  if (windSpeed <= 2) {
...
// 미세먼지 점수 계산
function calculateFineDustScore(fineDust: FineDust): { score: number; status: string } {
  switch (fineDust) {
    case '좋음':
...
// 자외선 지수 점수 계산
function calculateUvIndexScore(uvIndex: UvIndex): { score: number; status: string } {
  switch (uvIndex) {
    case '낮음':
